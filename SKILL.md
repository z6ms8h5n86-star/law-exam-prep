...
面向非技术用户（法学学生），按零依赖 → 完整能力三级渐进。

```
Tier 0: 零依赖（Claude 原生能力）              Tier 1: pip install         Tier 2: + 系统依赖
─────────────────────────────────────    ────────────────────────    ─────────────────────
• 文件发现 (bash find/ls/dir)             • .docx → 文本提取           • OCR 扫描件/图片
• .md / .txt 直接读取                     • .pptx → 文本提取           • 扫描 PDF → 文本
• .html 文本提取（Claude 读源码）          • .pdf (文本) → 提取         • PDF 输出 (WeasyPrint)
• 资料类型分类（Claude 推理）              • .epub → 文本提取
• 课程命名（用户对话）                     • 幕布 .json 解析
• 目录创建 (bash mkdir)                   • 大文件分段算法
• Markdown 输出 (Write tool)              • MD→HTML/PDF/DOCX
• 复习生成（Claude 原生推理）              • 去重算法
• 关键词匹配（Claude 文本分析）            • 编码检测 (chardet)
                                          • HTML→MD (markdownify)
```

**用户启动时自动检测**：如果 Python 不可用，skill 会提示"检测到你还没装 Python。要不要我帮你装？（winget/brew/apt 一键安装）"。

**降级策略**：每遇到一个需要 Python 的文件（如 .docx），skill 不会崩溃，而是：
- 标记该文件为 `[待转换: 需要 Python]`
- 继续处理其余 Tier 0 可处理的文件
- 最后汇总："X 个文件已处理，Y 个文件需要安装 Python 后转换。运行 `python setup.py` 一键安装环境？"
...
├─ PHASE 1: 多模态格式归一化
│  │
│  ├─ [1.0] 环境检测（Phase 1 开始前执行一次）
│  │  ├─ 检测 Python 是否可用：`python --version` 或 `python3 --version`
│  │  ├─ Python 可用？
│  │  │  ├─ YES → 检测 pip 依赖：`python -c "import docx; import PyPDF2; ..."`
│  │  │  │  ├─ 全部可用 → Tier 2 完整能力，继续
│  │  │  │  └─ 部分缺失 → 提示 "运行 pip install -r requirements.txt 获取完整能力"
│  │  │  │     └─ 用户选择：现在安装 / 跳过（仅 Tier 0）/ 稍后
│  │  │  └─ NO → 进入 Tier 0 降级模式
│  │  │     └─ 提示："检测到没有 Python 环境。Tier 0 模式：文本文件直接处理，"
│  │  │              "Word/PDF/PPT/图片将被标记为待转换。"
│  │  │              "要不要我帮你安装 Python？（winget/brew/apt 一键）"
│  │  │        ├─ 用户同意 → 执行 `python setup.py` 引导安装 → 重试
│  │  │        └─ 用户拒绝/跳过 → Tier 0 模式，继续
│  │  │
│  │  └─ Tier 标记写入 session context，后续所有 Phase 感知此状态
│  │
│  └─ 对每个用户确认的文件，按格式类型 + 当前 Tier 分发：
│     │
│     ├─ [1.1] Tier 0: 纯文本模态（无需 Python，Claude 原生处理）
│     │  │
│     │  ├─ .md / .txt → Claude 直接 Read 工具读取，自动检测 UTF-8/GBK
│     │  ├─ .html → Claude 读取后去除标签，保留标题层级
│     │  └─ .json（幕布导出）→ Claude 解析 JSON 结构，递归生成层级 MD
│     │
│     ├─ [1.2] Tier 1: 文档模态（需 Python）
│     │  │
│     │  ├─ Python 可用？
│     │  │  ├─ YES → `python tools/format_converter.py --input file.ext --mode xxx2md`
│     │  │  │  ├─ .docx → docx2md
│     │  │  │  ├─ .pptx → pptx2md
│     │  │  │  ├─ .pdf (文本) → pdf2md
│     │  │  │  └─ .epub → epub2md
│     │  │  └─ NO → 标记 [待转换: .docx 需 Python]
│     │  │     └─ 跳过该文件，继续下一个。Phase 6 汇总报告列出待转换清单
│     │  │
│     │  └─ PDF 文本/扫描判断：
│     │     ├─ PyPDF2 可用 → 尝试提取 → <100字符+多页 → 扫描件 → [1.4]
│     │     └─ PyPDF2 不可用 → 标记 [待检测: PDF 需 Python 判断类型]
│     │
│     ├─ [1.3] Tier 0: 上下文窗口估算（Claude 原生，无需 Python）
│     │  ├─ 中文 ≈ 字数 × 1.5, 英文 ≈ 词数 × 1.3
│     │  ├─ token < 80% 上下文 → 直接全量处理
│     │  └─ token ≥ 80% 上下文 → 按章节边界手动拆分（Claude 可识别 H1/H2 标题）
│     │
│     └─ [1.4] Tier 2: OCR 管线（需 Python + EasyOCR 或 Tesseract）
│        ├─ Python + EasyOCR 可用？
│        │  ├─ YES → `python tools/ocr_tool.py --input image.png --lang {按语言模式}`
│        │  │  ├─ 预处理 → OCR → 后处理（术语纠错 + 段落合并）
│        │  │  └─ 置信度标注
│        │  └─ NO → 标记 [待OCR: 需安装 pip install easyocr]
│        │     └─ 保留原图在 _原始文件/，后续可补 OCR
│        └─ 图像文件（.png/.jpg）：OCR 后进入文本池；OCR 不可用则跳过
│
