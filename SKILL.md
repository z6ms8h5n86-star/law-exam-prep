---
name: law-import
description: 法学资料导入器。自动发现微信/QQ下载的法学资料（或用户指定路径），多模态格式归一化（OCR/扫描PDF/图片/文档），逐文件资料类型分类（教材/笔记/重点/往年题/案例/术语表/阅读材料/论文/考纲/法条），由用户命名课程归属（关键词仅作智能建议），去重，按课程动态创建项目目录结构，支持多格式输出（MD/HTML/PDF/DOCX），大文件上下文窗口分段处理，可选连接law-review-generator生成复习材料。Use when user mentions "法学导入", "导入资料", "导入法学", "整理下载", "从微信整理", "从QQ整理", "批量导入笔记", "资料归档", "法学资料整理" or wants to import/collect law study materials from downloads.
---
...
```
多模态文件 → OCR/提取 → 统一文本 → 类型分类 → 学科归类 → 去重 → 项目结构 → 多格式输出
   │              │                    │           │         │         │           │
  图片           Tesseract          教材?       宪法学?    相似度   目录结构    MD/HTML
  扫描PDF        EasyOCR            笔记?       民法学?    字节      MOC        PDF/DOCX
  DOCX           python-docx        往年题?     刑法?     内容     索引       上下文分段
  PPTX           python-pptx        重点?       ...                             大文件
  EPUB           ebooklib           案例?                                    分块处理
  幕布JSON       递归解析            术语表?
  HTML           文本提取            论文?
  ...                               考纲?
                                    法条?
...
│  ├─ [6.2] 后续选项：
│  │  ├─ "仅归档" → 完成
│  │  ├─ "触发复习生成" → 桥接 law-review-generator
│  │  │  ├─ 有 syllabus → 直接作为 topic_map 骨架
│  │  │  ├─ 有 key_points → 补充考点权重
│  │  │  ├─ 有 past_exams → 标注高频考点
│  │  │  └─ 仅有笔记 → 按 H1/H2 标题自动生成 topic_map
│  │  ├─ "补充分类" → 对 low-confidence 文件重新手动分类
│  │  └─ "继续导入" → 返回 Phase 0 追加更多文件
│  │
│  └─ [6.3] 更新项目进度文件
│
...
```
law-import Phase 6 完成
  │
  ├─ 用户选择"触发生成复习"
  │
  ├─ 传递到 law-review-generator：
  │  ├─ syllabus 文件 → topic_map.json 骨架（考点+权重）
  │  ├─ key_points 文件 → 补充考点标记 + 【★重点】映射
  │  ├─ past_exams 文件 → 高频考点权重 + 题型分布
  │  ├─ textbook/notes 文件 → 源材料池（章节文本）
  │  └─ case_materials 文件 → 案例-考点关联嵌入
  │
  ├─ 课程名从 law-import 的 course 字段直接继承，无需重新命名
  │
  └─ law-review-generator 输出：
     ├─ review/{课程}/section_01.md ... section_NN.md（逐章复习稿）
     └─ review/{课程}/复习材料_终稿.md（合并版，可直接印PDF）
```
